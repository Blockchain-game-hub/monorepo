...
const verifyRequest = async (address: string, signature: string, nonce: string, setAuth) => {
  const response = await fetch(`/api/auth/verify?address=${address}&signature=${signature}&nonce=${nonce}`);
