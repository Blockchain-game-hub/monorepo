...
handler.use(middleware).post(async (
  ...
  try {
    ...
    jwtVerify(res, req.headers.authorization, async (err, decoded) => {
      ...
      const post = await prisma.post.create({data: {authorId: decoded.id, ipfsURL: cid, type: req.body.type[0], isPrivate: (req.body.isPrivate[0] === 'true'), title: req.body.title[0], description: req.body.description[0]}});
